...
class SaleTeamPerfKPIQty(models.TransientModel):
    _name = "st.perf.kpi.qty.wizard"
    _description = "Sale Team Performance By KPI Quantity Wizard"
        ...
        query = """
            ...
            SELECT 
                ...
                COALESCE(SUM(
                    CASE
                        WHEN lead.date_seq3 BETWEEN DATE %(start_date)s AND DATE %(end_date)s
                        AND lead.date_seq4 IS NOT NULL
                        THEN 1 ELSE 0
                    END
                ), 0) AS awarded_count
    ...
    def _print_sale_pipeline_xlsx(self, report_data):
        ...
        sheet = workbook.add_sheet("Sale Team Performance By KPI Quantity")
        ...
        # Row 1
        sheet.write_merge(1, 1, 0, 3, 'Sale Team Performance By KPI Quantity', cell_style_start)
        # Row 2
        ...
        for data in leads:
            ...
            projected_count = float(data.get('projected_count') or "0")
            awarded_count = float(data.get('awarded_count') or "0")
            lost_count = projected_count - awarded_count
            conversion_rate = ((projected_count - awarded_count) / projected_count) * 100 if projected_count else 0
            ...
            sheet.write(row, 6, conversion_rate, number_style)
        ...
        self.file_name = "Sale Team Performance By Kpi Quantity.xls"
