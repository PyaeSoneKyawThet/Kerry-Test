...
class SaleTeamPerformanceDetailReport(models.TransientModel):
    _name = "sale.team.performance.detail.wizard"
    _description = "Detail Sales Team Performance by Revenue"
    ...
    product_tmpl_ids = fields.Many2many('product.template', domain= "[('sale_ok', '=', True)]", string='Products')
    ...
    def _get_report_data(self):
        query = """
            ...
            WHERE so.is_job_order = TRUE
                  ...
                  AND pt.id IN %(product_tmpl_ids)s
        ...
        product_tmpl_ids = tuple(self.product_tmpl_ids.ids)
        ...
        if not product_tmpl_ids:
            params["product_tmpl_ids"] = tuple(self.env['product.template'].search([]).ids)
    ...
    def _print_sale_team_performance_xlsx(self, report_data):
        ...
        sheet = workbook.add_sheet("Detail Sales Team Performance By Revenue")
        ...
        # Row 1
        sheet.write_merge(1, 1, 0, 3, 'Detail Sales Team Performance by Revenue', cell_style_start)
        # Row 2
        ...
        for data in performance_data:
            ...
            sheet.write(row, 5, product_name, cell_style)
            sheet.write(row, 6, billing_amount, currency_style)
            sheet.write(row, 7, currency_symbol, cell_style)
            row += 1
            index += 1
        ...
        self.file_name = "Detail Sales Team Performance By Revenue.xls"
