...
class KMTLInvoiceQtyByCustomerWizard(models.TransientModel):
    _name = 'kmtl.invoice.qty.by.customer'
    _description = 'Invoice Qty by Customer Report Wizard'
    ...
    user_ids = fields.Many2many('res.users', string='Created User')
    ...
    def action_export_xlsx(self):
        ...
        sheet = workbook.add_worksheet('Invoice Qty by Customer')
        ...
        rowx += 1
        sheet.write(rowx, 0, 'Invoice Qty by Customer', title_format)
        rowx += 1
        ...
        sheet.write(rowx, 0, 'Currency : %s (Base Currency %s)' % (currency_name, currency_name), title_format)
        rowx += 1
        by_user = ', '.join(user.name for user in self.user_ids) if self.user_ids else 'All'
        sheet.write(rowx, 0, 'By User : %s' % by_user, title_format)
        rowx += 2
        ...
        fname = 'Invoice_Qty_by_Customer_%s.xlsx' % fields.Date.context_today(self)
        self.write({
