...
class KMTLARInvoiceTrackingState(models.Model):
    _name = 'kmtl.ar.tracking.state'
    _description = 'AR Invoice Tracking Report State'
    _order = 'sequence, id'
...
class KMTLARInvoiceTrackingReportWizard(models.TransientModel):
    _name = 'kmtl.ar.invoice.tracking.report'
    _description = 'AR Invoice Tracking Report Wizard'
    ...
    def _build_line_domain(self):
        ...
        domain = [
            ...
            # ('display_type', 'not in', ('line_section', 'line_note', 'tax')),
    ...
    def _analytic_key(self, line):
        if not line.analytic_distribution:
            return ()
        return tuple(sorted(
            (str(account_id), percentage)
    ...
    #     return abs(float(line.balance or 0.0))
    def _line_amount_in_currency(self, line):
        amount_currency = float(line.amount_currency or 0.0)
        if amount_currency:
    ...
    def _gather_data(self):
        ...
        #         line.partner_id.id,
        #         line.account_id.id,
        #         self._analytic_key(line),
        #     )
        #     amount_in_currency = self._line_amount_in_currency(line)
        #     grouped[group_key]['amount_in_currency'] += amount_in_currency
        #     if line.name:
        #         grouped[group_key]['descriptions'].add(line.name.strip())
        #     grouped[group_key].setdefault('line', line)
        ...
        for line in lines:
            ...
            amount_in_currency = self._line_amount_in_currency(line)
            # amount_in_currency = group['amount_in_currency']
            rows.append({
                'gl_date': move.date,
                'invoice_date': move.invoice_date,
                'due_date': move.invoice_date_due,
                'invoice_voucher_no': move.name or '',
                'customer_code': line.partner_id.ref or '',
    ...
    def action_export_xlsx(self):
        ...
        sheet = workbook.add_worksheet('AR Invoice Tracking Report')
        ...
        rowx += 1
        sheet.write(rowx, 0, 'AR Invoice Tracking Report', title_format)
        rowx += 1
        sheet.write(
            rowx,
            0,
        ...
        sheet.set_column(11, 11, 24)
        sheet.set_column(12, 12, 10)
        sheet.set_column(13, 15, 16)
        ...
        fname = 'AR_Invoice_Tracking_Report_%s.xlsx' % fields.Date.context_today(self)
        self.write({
