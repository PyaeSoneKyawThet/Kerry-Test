...
class KMTLInvoiceListReportWizard(models.TransientModel):
    _name = 'kmtl.invoice.list.report'
    _description = 'Invoice List Report Wizard'
    ...
    def _gather_data(self):
        ...
        if self.currency_ids:
            ...
            currency_ids = sorted({move.currency_id.id for move in moves if move.currency_id}, key=lambda cid: self.env['res.currency'].browse(cid).name)
        ...
        for move in moves:
            ...
            row = {
                'invoice_number': move.name or move.invoice_number or '',
    ...
    def action_export_xlsx(self):
        ...
        headers = ['No', 'Invoice Number', 'Customer Name', 'Sale PIC']
        for currency in currencies:
        ...
        for idx, row in enumerate(rows, start=1):
            ...
            colx += 1
            for currency in currencies:
                sheet.write(rowx, colx, row['amounts'].get(currency.id, ''), default_format if row['amounts'].get(currency.id, '') != '' else text_format)
                colx += 1
            rowx += 1
        ...
        fname = 'Invoice_List_Report_%s.xlsx' % fields.Date.context_today(self)
        self.write({
