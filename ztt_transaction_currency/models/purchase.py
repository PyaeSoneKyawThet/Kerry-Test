...
#                 self.picking_ids.move_ids_without_package.filtered(lambda x: x.purchase_line_id.id == line.id).price_unit = line.price_subtotal * self.currency_rate
...
class PurchaseOrderLine(models.Model):
    ...
    def _prepare_stock_move_vals(self, picking, price_unit, product_uom_qty, product_uom):
        ...
        # update unit price based on purchase uom and currency
        vals['price_unit'] = self.price_unit
        if self.currency_id != self.company_id.currency_id:
            price_unit_prec = self.env['decimal.precision'].precision_get('Product Price')
            currency_unit = self.currency_id._convert(
                self.price_unit, self.company_id.currency_id, self.company_id, self.order_id.date_order or fields.Date.today(), round=False)
            vals['price_unit'] = float_round(currency_unit, precision_digits=price_unit_prec)
        return vals
