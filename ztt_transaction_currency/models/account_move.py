...
class AccountMove(models.Model):
    ...
    @api.depends('currency_id','date','invoice_date')
    def compute_currency_rate(self):
        for rec in self:
            if rec.company_id.currency_id.id!=rec.currency_id.id:
                date = rec.invoice_date or rec.date or date.today()
    ...
    currency_rate = fields.Float('Currency Rate',compute='compute_currency_rate',store=True,readonly=False)
    ...
    # change in _compute_cash_rounding > diff_balance (no other change)
    # remove override 
...
class AccountMoveLine(models.Model):
    ...
    line_currency_rate = fields.Float('Line Currency Rate', store=True, related='move_id.currency_rate')
