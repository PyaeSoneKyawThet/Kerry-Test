...
class ApprovalExpense(models.Model):
    _name = 'approval.expense'
    _description = 'Approval Expense'
    ...
    purchase_order_id = fields.Many2one('purchase.order',string="Purchase Order Ref", tracking=True, copy=False)
    ...
    expense_line_ids = fields.One2many(comodel_name="approval.expense.lines", inverse_name="expense_id", string="Expense Lines")
    approver_ids = fields.One2many('approval.expense.approver', 'expense_id', string="Approvers", check_company=True,
    ...
    sub_total = fields.Float(string="Sub Total", compute="_compute_sub_total", help="Remaining amount")
    ...
    pay_to_id = fields.Many2one('hr.employee',string="Expense Pay to Employee", tracking=True)
    pay_to_external = fields.Char(string="Expense Pay to External", tracking=True)
    ...
    payment_id = fields.Many2one('account.payment', string="Current Payment", copy=False)
    payment_ids = fields.One2many('account.payment', 'approval_expense_id', string="Payments")
    ...
    bill_ids = fields.One2many('account.move', 'approval_expense_id', string="Bills")
    # === Amount fields === #
    ...
    amount = fields.Float(string="Total Amount", compute="_compute_total_amount", store=True, help="Total amount of the expense product lines")
    ...
    write_off_ids = fields.One2many('account.move', 'expense_id', string="Write Off")
    ...
    # check related "Cash Advance" is already used in "Payment Request"
    @api.depends('request_id')
    def _compute_show_is_cash_advance(self):
        for rec in self:
            approval_payment_request_ids = self.env['approval.payment.request'].search([('request_id','=',rec.request_id.id)])
            if approval_payment_request_ids and any(request.is_cash_advance for request in approval_payment_request_ids):
    ...
    def _compute_available_cash_advance_ids(self):
        for rec in self:
            # selected_expense = self.env['approval.expense'].search([('request_status','=','approved')])
            # selected_cash_advances = selected_expense.mapped('cash_advance_ids').ids
            # if selected_cash_advances:
            #     rec.available_cash_advance_ids = self.env['cash.advance.form'].search([('id', 'not in', selected_cash_advances),('request_id', '=', rec.request_id)]).ids
            # else:
            #     rec.available_cash_advance_ids = self.env['cash.advance.form'].searh([]).ids
            rec.available_cash_advance_ids = self.env['cash.advance.form'].search([('request_id', '=', rec.request_id.id),('is_clear','!=', True),('is_reimburse','!=', True)]).ids
    
    """if vendor_bill is approved
        : enable_direct payment: 
        : if no cash_advance, register vendor_bill
    ...
    def _compute_enable_payment(self):    
        for rec in self:
            ...
            if rec.request_status in ('approved') and rec.bill_state and rec.bill_state in ('posted') and rec.payment_state not in ('in_payment', 'paid'):
                ...
                rec.enable_cash_advance = False
                if (not self.payment_id or self.payment_id.state == 'cancel') and rec.bill_state and rec.bill_state in ('posted')and rec.sub_total < 0:
                    rec.enable_direct_payment = True
                else:
                    rec.enable_direct_payment = False
    ...
    def _compute_from_account_move_ids(self):
        for sheet in self:
            ...
            if sheet.move_id:
                sheet.amount_residual = sum(sheet.move_id.mapped('amount_residual'))
                sheet.payment_state = sheet.move_id[:1].payment_state
            else:
    ...
    def _compute_currency_id(self):
        for rec in self:
            rec.currency_id = rec.request_id.currency_id.id or self.env.company.currency_id.id
            
    @api.depends('expense_line_ids.total_amount', 'expense_line_ids.tax_amount')
    def _compute_total_amount(self):
        for rec in self:
            rec.untaxed_amount = sum(rec.expense_line_ids.mapped('total_amount'))
            rec.total_tax_amount = sum(rec.expense_line_ids.mapped('tax_amount'))
    ...
    def _compute_reimburse_amount(self):
        for rec in self:
            if rec.payment_id and rec.payment_id.state in ('posted'):               
                rec.reimburse_amount = rec.payment_id.amount * (-1) if rec.payment_id.payment_type == 'outbound' else rec.payment_id.amount
            else:
    ...
    def create(self, values):              
        for vals in values:
            if 'expense_line_ids' in vals:
                updated_expense_lines = []
                for val in vals['expense_line_ids']:
                    if isinstance(val, (tuple, list)) and len(val) > 2 and isinstance(val[2], dict) and 'product_id' in val[2]:
                        product_id = val[2]['product_id']
                        product = self.env['product.product'].browse(product_id)

                        update_data = val[2].copy()
                        if 'account_id' not in val[2]: 
                            update_data['account_id'] = product.property_account_expense_id.id or product.categ_id.property_account_expense_categ_id.id or False
                        if 'quantity' not in val[2]:
                            update_data['quantity'] = 1
                        if 'price_unit' not in val[2]:
                            update_data['price_unit'] = product.list_price
                        if 'product_uom_id' not in val[2]:
                            update_data['product_uom_id'] = product.uom_id.id

                        new_val = (val[0], val[1], update_data)
                        updated_expense_lines.append(new_val)
                    else:
                        updated_expense_lines.append(val)  

                vals['expense_line_ids'] = updated_expense_lines

            sequence = self.env['ir.sequence'].next_by_code('approval.expense.sequence')
            vals['name'] = "{}".format(str(sequence))
            if not 'request_owner_id' in vals:
                vals['request_owner_id'] = self.env.uid
        return super(ApprovalExpense, self).create(values)
    ...
    def write(self, vals):
        if 'expense_line_ids' in vals:
            ...
            vals['expense_line_ids'] = updated_expense_lines
        return super(ApprovalExpense, self).write(vals)
    ...
    @api.onchange('staff_location_id')
    def onchange_staff_location_id(self):
        for rec in self:
            rec.journal_id = self.env['account.journal'].search([('id','in',rec.available_journal_ids.ids)])[:1].id

    def _cancel_activities(self):
        approval_activity = self.env.ref('approval_extends.mail_activity_data_approval_expense_kmtl')
    ...
    def _compute_minimal_checker(self):
        for rec in self: 
            ...
            if rec.category_id.approval_type == 'expense':   
    ...
    @api.depends('category_id', 'request_owner_id', 'amount', 'currency_id', 'is_cash_advance')
    def _compute_approver_ids(self):
        for rec in self:
            ...
            if config.need_approval or rec.is_cash_advance:
                ...
                for approver in approver_approvers:                    
                    approver_id_vals.append(Command.create({
                        ...
                        'expense_id': self.id or self.ids[0],
                    }))
    ...
    def _compute_sub_total(self):
        for rec in self:
            ...
            if rec.amount:
                diff_amount =  rec.payment_id.diff_amount
    ...
    # remove in task id='2343'       
    def _get_user_approval_activities(self, user):
        domain = [
            ('res_model', '=', 'approval.expense'),
            ('res_id', 'in', self.ids),
            ('activity_type_id', '=', self.env.ref('approval_extends.mail_activity_data_approval_expense_kmtl').id),
            ('user_id', '=', user.id)
        ]
    ...
    def action_confirm(self):
        ...
        status = None
        if self.category_id.approval_type == 'expense':
            if self.config_id.need_approval or self.is_cash_advance:
                # remove this becasue journal_id is remove from expense form task:2559
                # if not self.journal_id.default_account_id and self.expense_line_ids:
                ...
                approvers = approvers[0] if approvers and approvers[0].status != 'to_check' else self.env['approval.expense.approver']
    ...
    def _update_next_checkers(self, new_status, checker, only_next_checker, cancel_activities=False):
        checkers_updated = self.env['approval.expense.approver']
        for approval in self:
    ...
    def _update_next_approvers(self, new_status, approver, only_next_approver, cancel_activities=False):
        approvers_updated = self.env['approval.expense.approver']
        for approval in self:
    ...
    #Expense Approved -> Vendor Bill
    def _prepare_account_move_vals(self):
        """prepare dictionary value to create vendor bill once expense form is approved"""
        l_vals = []  
        purchase_journal = self._get_purchase_journal()
        if not purchase_journal:  
            UserError(_("Please define vendor bills journal"))      
        ...
        vals = {'partner_id': partner.id,
                ...
                'approval_expense_id': self.id,
                'account_payment_type_id': self.payment_type_id.id,
                'staff_location_id': self.staff_location_id.id,
                'bill_type': "petty_cash_with_ca" if self.is_cash_advance else "petty_cash",
                'request_id': self.request_id.id,
    ...
    def _prepare_reconcile_move_vals(self):
        """prepare dictionary value to create move entry once expense form is approved
        ...
        vehicle_no = ', '.join(sorted(set(line.vehicle_no for line in self.expense_line_ids if line.vehicle_no)))  
        fmis_job_no = ', '.join(sorted(set(line.fmis_job_no for line in self.expense_line_ids if line.fmis_job_no))) 
        bl_no = ', '.join(sorted(set(line.bl_no for line in self.expense_line_ids if line.bl_no))) 
        reference_key = ', '.join(sorted(set(line.reference_key for line in self.expense_line_ids if line.reference_key))) 
        brand_name = ', '.join(sorted(set(line.brand_id.name for line in self.expense_line_ids if line.brand_id))) 
        ...
        l_vals = [
            # Debit Line(Payable)
            {
                ...
                'job_date': self.expense_line_ids[0].job_date if self.expense_line_ids else False,
                'bl_no': bl_no,
    ...
    def action_approve(self, approver=None):
        ...
        payable_line.sudo().write({'name': self.partner_id.id,
                                    'fmis_job_no': ', '.join(sorted(set(self.expense_line_ids.filtered(lambda x: x.fmis_job_no).mapped('fmis_job_no')))),
                                    'job_date': self.expense_line_ids[0].job_date if self.expense_line_ids else False,
                                    'vehicle_no': ', '.join(sorted(set(self.expense_line_ids.filtered(lambda x: x.vehicle_no).mapped('vehicle_no')))),
                                    'bl_no': ', '.join(sorted(set(self.expense_line_ids.filtered(lambda x: x.bl_no).mapped('bl_no')))),
                                    'reference_key': ', '.join(sorted(set(self.expense_line_ids.filtered(lambda x: x.reference_key).mapped('reference_key')))),
                                    'brand_name': ', '.join(sorted(set(self.expense_line_ids.filtered(lambda x: x.brand_id).mapped('brand_id').mapped('name')))),
                                    })
    ...
    def action_draft(self):
        ...
        self.mapped('bill_ids').write({'is_expense_cancel': False})
        self.write({'request_status': 'new',
                    'is_expense_cancel': False,
                    })
    ...
    def action_cancel(self):
        ...
        request_ids = self.mapped('request_id').filtered(
            lambda r: not r.cash_advance_ids and not r.payment_request_ids
        )
        ...
        if unused_request_ids:
            return {
                ...
                'target': 'new',           
                'context': {'default_approval_expense_ids': self.ids, 'default_request_ids': unused_request_ids.ids}            
            }
        else:
            self._action_cancel()
    ...
    def action_view_bill(self):
        return {
            ...
            'view_mode': 'tree,form',
            'domain': [('approval_expense_id', 'in', self.ids), ('move_type', 'in', ['in_invoice'])], 
        }
    ...
    def action_view_credit_note(self):
        return {
            ...
            'res_model': 'account.move',
            'view_mode': 'tree,form',
            'domain': [('approval_expense_id', 'in', self.ids), ('move_type', 'in', ['in_refund'])], 
        }
    ...
    def action_view_journal(self):
        return {
            ...
            'res_model': 'account.move',
            'view_mode': 'tree,form',
            'domain': [('approval_expense_id', 'in', self.ids), ('move_type', '=', 'entry'), ('auto_reconciled', '=', True)], 
        }
    ...
    def action_create_vendor_payment(self):
        if self.cash_advance_ids and not self.cash_advance_ids[0].product_id.property_account_expense_id:
    ...
    def action_register_payment(self):
        if self.sub_total > 0:
            ...
            """
            When: cash_advance amount is greater than expense line total amount
            Amount will remain with negative value
            Create: vendor payment for remaining amount
            """
    ...
    def _get_report_base_filename(self):
        self.ensure_one()
        return 'PETTY CASH VOUCHER-%s' % (self.name)  
    ...
    def action_print_clear_cash_advance(self):
        return self.env.ref('approval_extends.clear_ca_report').report_action(self)
    ...
    def action_view_payment(self):
        return {
            ...
            'view_mode': 'tree,form',
            'domain': [('approval_expense_id', 'in', self.ids)],
        }
    
    def export_data(self, fields_to_export, **kwargs):
        data = super(ApprovalExpense, self).export_data(fields_to_export, **kwargs)
...
class ApprovalExpenseLines(models.Model):
    _name = "approval.expense.lines"
    _description = "Approval Expense Detail lines"
    _inherit = ['mail.activity.mixin', 'analytic.mixin'] 
    _check_company_auto = True

    name = fields.Char(string="Name")
    expense_id = fields.Many2one('approval.expense', ondelete='cascade', index=True, copy=False)
    request_owner_id = fields.Many2one('res.users', related="expense_id.request_owner_id", string="Request Owner", store=True)
    ...
    date = fields.Date(related="expense_id.date")
    bl_no = fields.Char(string="BL No")

    category_id = fields.Many2one('product.category',string="Expense Product Category")
    ...
    tax_ids = fields.Many2many(
        ...
        relation='expense_line_tax',
        column1='expense_line_id',
        column2='tax_id',
        string="Taxes",
    ...
    def _compute_tax_ids(self):
        for line in self:
            expense = line.with_company(line.company_id)
        ...
        for line in self:
            ...
            if line.tax_ids:
                taxes_res = line.tax_ids.compute_all(
                    line.price_unit,
                    quantity=line.quantity,
                    currency=line.expense_id.currency_id
                )
    ...
    def _compute_currency_id(self):
        for line in self:
            line.currency_id = line.expense_id.currency_id.id
    ...
    @api.depends('date', 'company_id', 'currency_id',
        'total_amount_currency', 'product_id', 'quantity','price_unit')
    def _compute_total_amount(self):
        for expense in self:
    ...
    @api.depends('price_unit','quantity','total_amount', 'tax_ids')
    ...
    #         line.tax_amount = line.tax_amount_currency
    #         line.price_unit = line.total_amount / line.quantity if line.quantity else line.total_amount

    @api.onchange('product_id')
    def onchange_product_id(self):
        for line in self:
            line.account_id = line.product_id.property_account_expense_id.id or line.product_id.categ_id.property_account_expense_categ_id.id or False
